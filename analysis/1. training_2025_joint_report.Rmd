...
followed by professional staff members (`r round(df_attend %>% filter(role=="Professional") %>% select(n)/sum(df_attend$n)*100 ,0)`%).
Only `r round(df_attend %>% filter(role=="Student") %>% select(n)/sum(df_attend$n)*100 ,0)`% 
...
theme_sessions <- df_sp %>%
  group_by(theme) %>%
  select(c(theme, course_name, attended)) %>%
