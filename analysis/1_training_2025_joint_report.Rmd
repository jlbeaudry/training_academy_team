...
From the Flinders community, `r nrow(n_attendees_all_ienrol_hdr)` people engaged with the 
research training and development activities thus far in 2025. Our actual engagement numbers 
are larger (*n* = `r sum(df_all_sp_hdr$attended)`) because we allowed 
external people to join select sessions (e.g., Open Research sessions, 
...
demonstrating that a large number of people from the Flinders research community 
are engaging in our research training and development program.

These numbers include both synchronous sessions (*n* = `r nrow(n_courses_sync_ienrol_hdr)`), 
which include in-person, online, and hybrid sessions, and
asynchronous sessions (*n* = `r nrow(n_courses_async_ienrol_hdr)`), which are 
online modules that can be completed any time. Most of our participants attended the 
synchronous sessions (*n* = `r sum(df_sync_sp_hdr$attended)`), 
while `r sum(df_async_sp_hdr$attended)` engaged with the asynchronous sessions. 

The number of attendees at our synchronous sessions varied from `r (min(n_courses_sync_ienrol_hdr$n))` in 
some of the more bespoke HDR sessions (e.g., 3MT practice sessions, Drop-in sessions) 
to `r max(n_courses_sync_ienrol_hdr$n)` in our first session on Microsoft 
CoPilot and AI for research. The mean number of attendees at 
our sessions was `r round(mean(n_courses_sync_ienrol_hdr$n),0)` with a median of 
`r round(median(n_courses_sync_ienrol_hdr$n),0)`.
...
### Delivery Modality: Synchronous vs. Asynchronous -- I still need to pull this out

Most of the `r nrow(n_courses_all_ienrol_hdr)` sessions that we offered in 2025 were 
offered as synchronous sessions 
(n = `r n_courses_sync_ienrol_hdr %>% count()`) 
whereas others were offered as asynchronous
(n = `r n_courses_async_ienrol_hdr %>% count()`; typically
as online modules through external providers such as Epigeum). 
...
(*n* = `r nrow(n_courses_rds)-nrow(n_event_sp)`; `r round(((nrow(n_courses_rds)-nrow(n_event_sp))/(nrow(n_courses_rds))*100),0)`%) were offered online through synchronous delivery. 
...
of synchronous sessions and attendees by theme. 
...
```{r sync sessions and attendees by theme, include = TRUE, eval=TRUE}
...
### Themes for Asynchronous Training & Development Programs
...
they meet the needs of international and part-time students. Unfortunately, due
to difficulties within the system, we cannot provide data for student engagement 
with the online modules. 

The RDS team has 
offered the Epigeum 'Research Integrity' modules to the research community, but 
uptake has been limited thus far. As such, we have not included the data in this
report. 


\newpage

```{r async theme overview, include = TRUE, eval = TRUE}
...
# how many courses per theme
async_theme_sessions <- df_async_sp_hdr %>%
  group_by(theme) %>%
  select(c(theme, course_name, team, attended)) %>%
  arrange(theme, course_name) %>%
  mutate(theme = as.factor(theme))

# drop the unused levels (all the RDS themes)
async_theme_sessions$theme <- droplevels(async_theme_sessions$theme)

# make it look good
kable(async_theme_sessions [, 2:4],
  booktabs = T,
  linesep = "\\addlinespace",
  col.names = c("Sessions by Themes", "Team", "Number of Attendees"),
  caption =
  ...
  column_spec(2, width = "0.7in") %>%
  column_spec(3, width = "1in") %>%
  pack_rows (index = table(async_theme_sessions$theme),
               latex_gap_space = "0.4em",) %>%
    kable_styling(
    full_width = F,
    latex_options = c("repeat_header", "hold_position", "striped")
