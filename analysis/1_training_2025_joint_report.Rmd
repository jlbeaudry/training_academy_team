...
```{r child = here::here("analysis", "0_cleaning_2025_training_data.Rmd"), include=FALSE, eval=FALSE}
...
# Calculate how many sessions people attended

attendance_times <- df_sync_ienrol_hdr %>%
  group_by(fan) %>%
  summarise(sessions_attended = n(), .groups = "drop") %>% 
  count (sessions_attended, name = "number_of_people")
...
fig_nums(name = "attendance_times", 
         caption = "Distribution of the Number of Synchronous Sessions People Atttended (2025)")
...
table_nums(name = "async_theme_overview", 
          caption = "Training and Development Summary for Asynchronous Modules (2025)")

table_nums(name = "async_sessions_by_theme", 
          caption = "Training and Development Asynchronous Modules by Theme (2025)")
...
**`r nrow(n_courses_all_ienrol_hdr)` information sessions and development workshops** 
to researchers, research support staff, HDR supervisors and HDR students at Flinders
University. 

From the Flinders community, **`r nrow(n_attendees_all_ienrol_hdr)` people** have 
engaged with our research training and development activities thus far in 2025, 
demonstrating that 
...
(*n* = `r nrow(n_courses_sync_ienrol_hdr)`; e.g., in-person, 'live' online, and hybrid sessions) and 
asynchronous online modules (*n* = `r nrow(n_courses_async_ienrol_hdr)`) that HDR supervisors
could complete any time. Please note that HDR student engagement in online modules
is not captured in this report due to limitations in system reporting. 
Most of our participants engaged with 
...
`r attendance_summary$max_sessions`. As illustrated in `r fig_nums(name = "attendance_times", display = "cite")`, 
the distribution of the number of sessions attended is positively skewed.
The number of 'repeat customers' speaks to the quality of the sessions on offer. 
...
`r fig_nums(name = "attendance_times")`


```{r plot the attendance distribution, include = TRUE, eval=TRUE, fig.width = 15, fig.height = 9}


attendance_times %>%
  ggplot(aes(x = sessions_attended, y = number_of_people)) +
    geom_col(fill = "#90b8da", color = "black") +
  geom_text(aes(label = number_of_people), vjust = -0.5) +
  labs(x = "Sessions Attended",
       y = "Frequency") +
  theme_classic() +
    theme(
...
The patterns of the data in `r fig_nums(name = "college_attendees", display = "cite")` 
and `r fig_nums(name = "college_unique_attendees", display = "cite")` broadly reflect the size of the 
Colleges. 
...
Of the `r nrow(n_courses_rds)` sessions that RDS offered thus far in 2025, `r nrow(n_event_sp)` 
...
```{r sync attendance by role, eval=TRUE}
...
(`r round(df_attend %>% filter(role=="Student") %>% select(n)/sum(df_attend$n)*100 ,0)`%), and 
...
```{r plot the sync attendance by role, include = TRUE, eval=TRUE, fig.width = 15, fig.height = 9}
...
Most of the `r nrow(n_courses_all_ienrol_hdr)` activities that we held were 
...
### Attendance Patterns and Numbers
...
Next, we explore the topics and modalities of the sessions in more detail. 

We first report the synchronous sessions offered by RDS and OGR, then the 
asynchronous modules offered by OGR to HDR students and supervisors.
...
to supporting and developing researchers so they can attract external research funding. 
...
additional detail about the modules that HDR students and supervisors completed.
...
Nearly all asynchronous activities were offered by OGR because they need to ensure
...
synchronous), a range of online modules and seminars oused on Canvas, and numerous 
...
kable(async_theme_overview,
      ...
      align = c('lcc'),
      caption = "Summary of Asynchronous Modules for HDR Students and HDR Supervisors (2025)",
      linesep = "\\addlinespace") %>%
  kable_styling(latex_options = c("hold_position", "striped"))
...
kable(async_theme_sessions [, 2:3],
  ...
  caption =
    sprintf("Asynchronous Modules for HDR Students and HDR Supervisors (n = %d)",
...
```{r other sessions and attendees, include = TRUE, eval = FALSE}
